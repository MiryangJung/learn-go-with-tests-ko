...
행복한 경로 코드가 준비되었으니 이제는 약간 더 현실성이 있는 경우를 가정해 볼 차례이다. 사용자가 요청을 취소하기 전까지 `Store`가 `Fetch`를 끝내지 못하는 경우를 생각해보자.
...
스파이를 생성할 때 `*testing.T`를 잊지 말고 넘겨주도록 하자.
...
스파이를 수정하여 `context`를 실제로 사용하는 메서드처럼 바꿔보자.
